  ...
  EventProcessors = {
    ...
    'input_audio_buffer.speech_stopped': (event, inputAudioBuffer) => {
      ...
      if (speech) {
         ...
         console.error('speech object is undefined')
      }
      if (inputAudioBuffer) {
    ...
    'response.output_item.done': (event) => {
      const { item } = event;
      if (!item) {
        console.error(`response.output_item.done: Missing "item"`);
      }
    ...
    'response.audio.delta': (event) => {
      ...
      const item = this.itemLookup[item_id];
      if (!item) {
        console.error(`response.audio.delta: Item "${item_id}" not found`)
      }
    ...
    'response.text.delta': (event) => {
      ...
      const item = this.itemLookup[item_id];
      if (!item) {
        console.error(`response.text.delta: Item "${item_id}" not found`);
      }
